...
class AccountMoveLineImport(models.TransientModel):
    ...
    csv_separator = fields.Selection(
        [(',', ', (comma)'), (';', '; (semicolon)')],
        string='CSV Separator', required=True, default=';')
    ...
    def _handle_debit(self, field, line, move, aml_vals):
        if 'debit' not in aml_vals:
            ...
            debit = self.env.user.company_id.currency_id.round(debit)
    ...
    def _handle_credit(self, field, line, move, aml_vals):
        if 'credit' not in aml_vals:
            ...
            credit = self.env.user.company_id.currency_id.round(credit)
    ...
    def _handle_partner(self, field, line, move, aml_vals):
        if not aml_vals.get('partner_id'):
            ...
            if line.get('vat_code', False):
                domain = dom + [('vat', '=', line['vat_code'])]
                partners = part_mod.search(domain)
            else:
                partners = False
            if not partners and line.get('partner_code', False):
                domain = dom + [('kodas', '=', line['partner_code'])]
                partners = part_mod.search(domain)
            if not partners:
            ...
            if not partners and line.get('changed_partner'):
                dom_name = dom + [('name', '=', line['changed_partner'])]
                partners = part_mod.search(dom_name)
            if not partners:
    ...
    def _process_vals(self, move, vals):
        ...
        if round(self._sum_debit, dp) != round(self._sum_credit, dp):
            self._err_log += '\n' + _(
                "Klaida CSV file, debetas (%s) is "
                "skiriasi nuo kredito (%s)! Taip gali būti dėl nesuapvalintų įrašų faile"
    ...
    def aml_import(self):
        ...
        if self._header_fields and self._header_fields[0] and self._header_fields[0][:3] == '\xEF\xBB\xBF':
            self._header_fields[0] = self._header_fields[0][3:]
