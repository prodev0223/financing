...
supported_currecies = {
    ...
    'LB_getter': LB_supported_currency_array,
    }
...
class Currency_rate_update_service(models.Model):
    ...
    def _check_max_delta_days(self):
        for rec in self:
            if rec.max_delta_days < 0:
                raise exceptions.ValidationError(_('Max delta days must be >= 0'))
    ...
    def _check_interval_number(self):
        for rec in self:
            if rec.interval_number < 0:
                raise exceptions.ValidationError(_('Interval number must be >= 0'))
    ...
    def _onchange_interval_number(self):
        if self.interval_number == 0:
            self.note = '%s Service deactivated. Currencies will no longer ' \
                        'be updated. \n%s' % (datetime.utcnow().strftime(tools.DEFAULT_SERVER_DATETIME_FORMAT),
