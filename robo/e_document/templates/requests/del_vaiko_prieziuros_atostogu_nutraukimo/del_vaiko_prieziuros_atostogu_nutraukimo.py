...
class EDocument(models.Model):
    ...
    @api.multi
    def prasymas_del_vaiko_prieziuros_atostogu_nutraukimo_workflow(self):
        self.ensure_one()
        generated_id = self.env['e.document'].create({
            'template_id': self.env.ref('e_document.isakymas_del_vaiko_prieziuros_atostogu_nutraukimo_template').id,
