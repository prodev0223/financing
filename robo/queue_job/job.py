...
class Job(object):
    ...
    def store(self):
        ...
        if db_record:
            with db_record.env.norecompute():
                db_record.with_context(recompute=False).write(vals)
        else:
    ...
    @property
    def func(self):
        recordset = self.recordset.with_context(job_uuid=self.uuid, method_running_as_job=True)
    ...
    def _get_retry_seconds(self, seconds=None):
        ...
        if not seconds and retry_pattern:
            ...
            patt = sorted(iteritems(retry_pattern), key=lambda t: t[0])  # P3:Ok
