...
class QueueJob(models.Model):
    ...
    @api.model
    def cron_requeue_stuck_document_after_redeployment(self):
        """ Mark job that were started before system restart as failed """
        now = datetime.now(pytz.timezone('Europe/Vilnius'))
        restart = now + relativedelta(hour=20, minute=55)
        if restart > now:
            restart += relativedelta(days=-1)

        stuck_jobs = self.search([
            ('state', '=', 'started'),
            ('date_started', '<=', restart.astimezone(pytz.utc).strftime(tools.DEFAULT_SERVER_DATETIME_FORMAT)),
        ])
