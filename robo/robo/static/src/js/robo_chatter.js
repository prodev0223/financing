robo.define('robo.Chatter', function (require) {
"use strict";
...
//
// ROBO: copy-paste from mail.Chatter; change in preprocess_message
// -----------------------------------------------------------------------------
var ChatterComposer = composer.BasicComposer.extend({
    template: 'robo.ChatComposer',
    ...
    init: function (parent, dataset, options) {
        ...
        if (this.options.is_log) {
            this.options.send_text = _t('Registruoti');
        }
    ...
    //ROBO: we need to generate only front messages with action_id and rec_id
    preprocess_message: function () {
        ...
        this._super().then(function (message) {
            message = _.extend(message, {
                ...
                subject: 'Komentaras',
                robo_rec_id: self.getParent().view.datarecord.id || self.context.default_res_id, //ROBO: parent is abstract field
                robo_action_id: (self.getParent().context && self.getParent().context.related_front_action) ||
                                (self.getParent().view.options.action && self.getParent().view.options.action.id),
            ...
            // Client message
            if (self.options.client_message) {
                message.client_message = self.options.client_message;
            }

            if (self.options.ticket_close) {
                message.ticket_close = self.options.ticket_close;
            }
...
var Chatter = form_common.AbstractField.extend({
    template: 'robo.Chatter',

    events: {
        "click .o_chatter_button_new_message": "on_open_composer_new_message",
        "click .o_chatter_button_new_message_client": "on_open_composer_new_client_message",
        "click .o_chatter_button_new_message_client_ticket": "on_open_composer_new_client_ticket_message",
    ...
    willStart: function () {
        var self = this;
        return $.when(session.is_user(), session.is_accountant()).then(function (is_user, accountant){
    ...
    start: function () {
        ...
        this.thread = new ChatThread(this, {
            ...
            display_stars: false, //ROBO: do not display stars
        });
    ...
    on_open_composer_new_message: function () {
        this.open_composer({client_message: false});
    },

    on_open_composer_new_client_message: function () {
        this.open_composer({client_message: true});
    },
    on_open_composer_new_client_ticket_message: function () {
        this.open_composer({client_message: true, ticket_close: true});
    },
    on_open_composer_log_note: function () {
        this.open_composer({client_message: false, is_log: true});
    },
    open_composer: function (options) {
        ...
        this.composer = new ChatterComposer(this, this.thread_dataset, {
            ...
            client_message: options && options.client_message,
            ticket_close: options && options.ticket_close,
...
core.form_widget_registry.add('robo_mail_thread', Chatter);
