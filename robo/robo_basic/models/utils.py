...
from validate_email import validate_email as validate_email_rfc
import re
...
def validate_email(email, verify=False, check_mx=False):
    # Add a regex check for a '.' in domain - everything else is validated by RFC 2822
    if not validate_email_rfc(email, verify=verify, check_mx=check_mx) or not re.match(r'^.*@.*\..*$', email):
        return False
    return True
