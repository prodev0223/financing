...
class MailTemplatePreview(models.TransientModel):
    ...
    def _get_records(self):
        ...
        if template.model_id.model == 'account.invoice':
            records = self.env[template.model_id.model].search([('type', 'in', ['out_invoice'])], limit=10)
        else:
    ...
    def on_change_res_id(self):
        ...
        for field in ['email_from', 'email_to', 'email_cc', 'reply_to', 'subject', 'body_html', 'partner_to',
