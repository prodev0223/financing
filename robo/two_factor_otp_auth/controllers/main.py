...
from odoo.addons.auth_brute_force.controllers.controllers import LoginController
from odoo.addons.passwords.controllers.main import PasswordSecurityHome
from odoo.http import request

from ..exceptions import MissingOtpError, InvalidOtpError


class Login2fa(LoginController):
    ...
    def web_login(self, redirect=None, **kw):
        ...
        if query_string and 'token' in query_string and not redirect:
            redirect = '/web#'
        try:
...
class PasswordSecurityHome2FA(PasswordSecurityHome):

    @http.route()
    def web_auth_reset_password(self, *args, **kw):
        try:
            response = super(PasswordSecurityHome2FA, self).web_auth_reset_password(*args, **kw)
        except MissingOtpError:
            # user will get into this block if login process is not fully successful
            # For example, when first login was successful, but 2FA token is missing
            # So we can start second authentication step (OTP)
