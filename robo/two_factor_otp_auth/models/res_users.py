...
class ResUsers(models.Model):
    ...
    def __init__(self, pool, cr):
        ...
        type(self).SELF_WRITEABLE_FIELDS.extend(['enable_2fa'])
        type(self).SELF_READABLE_FIELDS = list(self.SELF_READABLE_FIELDS)
        type(self).SELF_READABLE_FIELDS.extend(['enable_2fa'])
        return init_res
    ...
    def check_credentials(self, password):
        ...
        if self.enable_2fa and request:
    ...
    def _get_otp_provision_uri(self):
        """
        Overridable method to show that returns string that is shown on OTP app
        :return:
        """
        return 'RoboLabs ({})'.format(self.login)
    ...
    def _generate_secrets(self):
        ...
        provisioning_uri = self._get_otp_provision_uri()
        code = pyotp.totp.TOTP(key).provisioning_uri(provisioning_uri)
    ...
    @api.multi
    def _can_change_2f_auth_settings(self, user):
        """
        Check that user can change 2FA settings on the RecordSet
        Argument:
        * user - `res.users` object
        ...
        """
        if any(u != user for u in self) and not user.has_group("two_factor_otp_auth.mass_change_2fa_for_users"):
            raise AccessError(_(
                "Only users with 'Mass Change 2FA Configuration for Users' rights can do this operation!"
            ))
